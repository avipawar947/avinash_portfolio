...
import { PROJECT_MIX } from "@/lib/constants";

/**
 * "On Average Projects" — Figma node 1:312.
 *
 * 530 x 424, radius 8, flat #1B1B1B (the design stacks two gradients,
 * the upper of which is opaque #1B1B1B end to end, so the lower one
 * never shows). A 470 x 276 plot area at (34, 80) holds four bars over a
 * vertical rule, with a horizontal axis at y=356 labelled 0 and 100.
 *
 * Bars are the design's own pixel widths against that 447px axis.
 */
export function ProjectMixCard() {
  return (
    <div
    ...
    >
      ...
      >
        {/* Vertical rule the bars grow from (1:327).
            The export is a plain 1px #D0D0D0 hairline at 20% opacity, so
            it is drawn in CSS: an <img> scaled on one axis only trips
            next/image's aspect-ratio warning, and a hairline stays
            crisper as a border than as a stretched bitmap. */}
        <div
          aria-hidden="true"
