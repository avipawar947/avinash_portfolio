 ...
 *
 * The count-up preserves the rendered digit count ("04" stays two digits)
 * so the type does not reflow as it counts, and is skipped entirely under
 * reduced motion — where the final figure renders from the first frame.
 */
export default function StatValue({ value, suffix }: { value: string; suffix: string }) {
  ...
  return (
    <span
      className="absolute flex items-end font-display font-medium text-[var(--color-text)]"
      style={{
